...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_cart'])) {
            ...
            if ($new_quantity > 0) {
                ...
                if ($stock !== false) {
                    if ($new_quantity <= $stock) {
                        ...
                        $update_message = "Some items were adjusted to maximum available stock.";
    ...
    header("Location: cart.php?status={$update_status}&message=" . urlencode($update_message));
    exit;
}

// Handler for removing a single item
if (isset($_GET['action']) && $_GET['action'] == 'remove' && isset($_GET['id'])) {
    $id = filter_var($_GET['id'], FILTER_VALIDATE_INT);
    if ($id && isset($_SESSION['cart'][$id])) {
        unset($_SESSION['cart'][$id]);
        header("Location: cart.php?status=success&message=Item removed from cart.");
        exit;
    }
}
...
<head>
    ...
    <style>
        ...
        .header { background-color: var(--color-primary); color: white; padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; position: fixed; top: 0; width: 100%; box-sizing: border-box; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); z-index: 100; }
        ...
        .btn-remove { color: var(--color-danger); background: none; border: none; cursor: pointer; font-size: 1.1rem; transition: color 0.2s; }
    ...
    <div class="container">
        ...
        <?php if (count($cart) > 0): ?>
            <form method="POST" action="cart.php">
                        ...
                        <?php foreach ($cart as $id => $item): ?>
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <a href="cart.php?action=remove&id=<?= $id ?>" class="btn-remove" title="Remove Item" onclick="return confirm('Remove this item from cart?');"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <div class="cart-actions">
                    <a href="store.php" class="btn btn-back"><i class="fas fa-arrow-left"></i> Continue Shopping</a>
        ...
        <?php else: ?>
            <div class="alert alert-info" style="text-align: center;">
                ...
                <p style="margin-top: 15px;"><a href="store.php" class="btn btn-primary" style="background-color: var(--color-primary); color: white;">Start Shopping</a></p>
            </div>
        <?php endif; ?>
    </div>
</body>
